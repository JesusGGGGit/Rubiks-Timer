...
function calculateStats(times, plusTwoTimes, dnfTimes) {
...
useEffect(() => {
  const currentTime = stats.numericTimes[stats.numericTimes.length - 1];

  if (
    activeSession.times.length > 0 &&
    currentTime !== undefined &&
    prevBestTimeRef.current !== null &&
    currentTime < prevBestTimeRef.current
  ) {
    fireConfetti();
  }
  prevBestTimeRef.current = stats.bestTime;
}, [activeSession.times, stats.bestTime, stats.numericTimes, fireConfetti]);
